...
function StartView:init()
	...
	if ComMgr:getInstance():getData(CmdName.Frist_Game) ~= 1 then
		...
		ComWinTips.show("第一次进入游戏，赠送8888金币!", function()
			ComMgr:getInstance():setData(CmdName.Game_Icon, 8888)
...
function StartView:checkLogin()
	local oldDay = ComMgr:getData(CmdName.Game_Day)
	local oldMon = ComMgr:getData(CmdName.Game_Mon)
	local curDay, curMon = ComMgr:getInstance():getNowTime()
	if oldDay ~= curDay or oldMon ~= curMon then
		ComWinTips.show("每日登录，赠送1000金币!\n有上就送，天天上，天天送！", function()
			local curCoin = ComMgr:getInstance():getData(CmdName.Game_Icon)
			ComMgr:getInstance():setData(CmdName.Game_Day, curDay)
			ComMgr:getInstance():setData(CmdName.Game_Mon, curMon)
			ComMgr:getInstance():setData(CmdName.Game_Icon, curCoin + 1000)
			self:startInit()
			self:removeChildByTag(CmdName.Com_Win_Tips, false)
		end, self)
	else
